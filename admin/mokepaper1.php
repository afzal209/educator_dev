...
<?php
	...
	if ($current_date == $date) {
		if($current_time >= $start_paper && $current_time <= $end_paper){
							...
							<div class="single-post row">
								...
								");
									...
									if(mysqli_num_rows($query) == 0){ 
                                        ...
                                        $row = mysqli_fetch_assoc($query);
                                        $r = $row['id'];
										echo ' 	<h1 class="text-center" style="font-family: jameel noori nashtalik;">Notice</h1>
												<h2 class="text-center" style="font-family: jameel noori nashtalik;">Congratulations</h2>
										...
										while($row=mysqli_fetch_array($query)){
											...
											echo'
                                            <h3 class="text-center" style="font-family: jameel noori nashtalik;font-size:30px">Moke Paper</h3>
                                            <h3 style="text-align: right; font-family: jameel noori nashtalik;" id="countdown"></h3>
                                            <h3 style="text-align: right; font-family: jameel noori nashtalik;">Date:'.$row['date'].'</h3>
											<h3 style="font-family: jameel noori nashtalik;font-size:30px">'.$row['job_title'].'</h3>
                                                ...
                                                ';
                                                        for ($i=1; $i<=4 ; $i++) { 
                                                            ...
                                                            <div class="col-lg-6 col-md-12 col-sm-12">
                                                                <button type="button" class="genric-btn primary-border e-large btn_size"  data-option="'.$row['option'.$i].'" id="option'.$i.'" name="option'.$i.'" onclick="SubmitQuestion(this,\'mokepaper.php?id='.@$id.'&page='.(@$page+1).'\');" >'.$row['option'.$i].'</button>	
                                                            </div>
                                                            ';	
                                                        }
                                                        echo '
